...
Start centos docker container
    sudo docker run -d -t --name centostest1 centos
list container
    docker ps
run container
    sudo docker exec -it centostest1 bash
exit

#Quick test alpine
    sudo docker pull alpine
Start alpine docker container
    sudo docker run -d -t --name aplinetest1 alpine
run container
    sudo docker exec -it aplinetest1 sh

#Network chuck example
sudo docker pull thenetworkchuck/nccoffee:frenchpress
sudo docker run -d -t -p 80:80 --name coffee1 thenetworkchuck/nccoffee:frenchpress
...
kubectl create deployment test1 --image=thenetworkchuck/nccoffee:frenchpress
kubectl expose deployment test1 --type=NodePort --port=80

minikube service test1 --url
http://172.17.0.3:31320


delete
kubectl delete services test1
kubectl delete deployment test1
minikube stop
minikube delete

kubectl scale --replicas=2 deployment/test1
kubectl get pods
```
